...
import static imp.controladoresview.CotizacionViewModel.fechaEmision;
 ...
 * @author gato
 */
public class CotizacionCompaginadoViewModel1 {

    @Wire
    Window windowClienteBuscarCom;
    @Wire
    ...
    public CotizacionCompaginadoViewModel1() {
    ...
    private void armarCotizacion() {
        try {
            ...
            encabezado.setCotTipoCotizacion("CIC");
            ...
            for (DescripcionPedido item : listaPedido) {
                ...
                detAuxMaterial.setDetTipoTrabajo(item.getDetTipoTrabajo());
...
//            det.setDetMotivo(item.getMotivo().toString());
            ...
            //System.out.println("Longitud de varios " + listaPedidoVarios.size());
...
//para pasar al visor
    ...
    public void nuevaDescripcionGeneral() {
        ...
        if (materialSeleccionadoCombo.getMatNombre() != null
            ...
            nuevoRegistro.setDetTipoTrabajo(tipoTrabajo.getTipoDescripcion());
...
//            calcularValorTotalesGeneral();
    ...
    private void calcularValorTotalesGeneral() {
        ...
        List<DescripcionPedido> listaPedido = listaDescripcionPedido.getInnerList();
        System.out.println("lista " + listaPedido.size());
        ...
        if (listaPedido.size() > 0) {
            for (DescripcionPedido item : listaPedido) {
                ...
                BigDecimal porcentajeDescuento = item.getPorcentajeDescuentoMaterial().divide(BigDecimal.valueOf(Long.valueOf(100)), 4, RoundingMode.FLOOR);
                BigDecimal valorDescuento = valorPliegos.multiply(porcentajeDescuento);
                valorPliegos = valorPliegos.add(valorDescuento.negate());
//                BigDecimal numeroMillares = item.getCantidad().multiply(item.getPliegosImpresionCompaginado());
                BigDecimal hojasUxF = BigDecimal.ZERO;
                if (item.getUxf().intValue() > 0) {
                    hojasUxF = item.getMotivo().divide(item.getUxf(), 0, RoundingMode.UP);
                }
                System.out.println("hojasUxF " + hojasUxF);
...
//                    cantidadTotMillares = numeroMillares;
                BigDecimal cantidadPorHojas = item.getCantidad().multiply(item.getMotivo());
                cantidadPorHojas.setScale(0, RoundingMode.UP);
                BigDecimal cantidadSobreMil = cantidadPorHojas.divide(item.getUxf(), 6, RoundingMode.FLOOR);
                cantidadSobreMil = cantidadSobreMil.divide(BigDecimal.valueOf(1000), 6, RoundingMode.FLOOR);
                System.out.println("cantidadSobreMil " + cantidadSobreMil);
//                numeroMillares = numeroMillares.divide(BigDecimal.valueOf(1000));


                costoPlacas = costoPlacas.add(item.getPlacas().multiply(BigDecimal.valueOf(item.getManoObra().getManCostoPlancha())));
                if (item.getTipoCobroManoObra().equals("M")) {

                    if (cantidadSobreMil.doubleValue() >= 1) {


                        BigDecimal valorCalculoDecimal = BigDecimal.ZERO;
//                        valorCalculoDecimal = cantidadSobreMil.add(cantidadSobreMil.setScale(0, BigDecimal.ROUND_DOWN).negate());
//                        //System.out.println("decimal verificacion " + valorCalculoDecimal);
//                        if (valorCalculoDecimal.doubleValue() >= Double.valueOf(0.2)) {
//                            cantidadSobreMil = cantidadSobreMil.setScale(0, BigDecimal.ROUND_UP);
//                        } else {
//                            cantidadSobreMil = cantidadSobreMil.setScale(0, BigDecimal.ROUND_DOWN);
//                        }
                        BigDecimal millarTotal = item.getPliegos().multiply(BigDecimal.valueOf(item.getCortezPosibles().getCodFxp()));
                        millarTotal = millarTotal.divide(BigDecimal.valueOf(1000.0));

                        System.out.println("TIOTAL MILLARES " + millarTotal);
                        item.setCantidadHoraMillar(millarTotal);
//                            BigDecimal valorManObraGen = millarTotal.multiply(BigDecimal.valueOf(item.getManoObra().getManCostoMillar()));
                        BigDecimal sumaAnvRev = BigDecimal.valueOf(item.getColorAnverso()).add(BigDecimal.valueOf(item.getColorReverso()));
                        System.out.println("ANV + REV " + sumaAnvRev);

                        BigDecimal costoPorColor = sumaAnvRev.multiply(BigDecimal.valueOf(item.getManoObra().getManCostoMillar()));
                        //System.out.println("ANV + REV POR COSTO " + costoPorColor);
                        BigDecimal costoTotalMillarcostoPorColor = millarTotal.multiply(costoPorColor);
                        System.out.println("TOTAL MANO OBRA " + costoTotalMillarcostoPorColor);
//                            valorManoObra = valorManoObra.add(valorManObraGen.multiply(BigDecimal.valueOf(item.getColorAnverso() + item.getColorReverso())));
//                            valorManoObra = valorManoObra.multiply(sumaAnvRev);
                        //aplica el descuento
                        valorManoObra = valorManoObra.add(costoTotalMillarcostoPorColor.add(((costoTotalMillarcostoPorColor.multiply(item.getDescuentoMano())).divide(BigDecimal.valueOf(Long.valueOf("100")))).negate()));
                        //
...
//                        cantidadTotMillares = cantidadTotMillares.multiply(hojasUxF);
...
//                        BigDecimal sumaAnvRev = BigDecimal.valueOf(item.getColorAnverso()).add(BigDecimal.valueOf(item.getColorReverso()));
//                        BigDecimal costoPorColor = sumaAnvRev.multiply(BigDecimal.valueOf(item.getManoObra().getManCostoMillar()));
//                        BigDecimal costoTotalMillarcostoPorColor = cantidadTotMillares.multiply(costoPorColor);
...
//                        item.setValorTotalManoObra(valorManoObra);
                    ...
                    BigDecimal motivosSobreUxF = item.getMotivo().divide(item.getUxf(), 4, RoundingMode.FLOOR);
                    BigDecimal cantidadSobreUxF = item.getCantidad().add(item.getMaculatura());
                    BigDecimal cantidadMillar = motivosSobreUxF.multiply(cantidadSobreUxF);

                    BigDecimal numeroHoras = cantidadMillar.divide(BigDecimal.valueOf(item.getManoObra().getManImpresionHora()), 2, RoundingMode.DOWN);
                    //System.out.println("NUMERO DE HORAS--->  " + numeroHoras);
                    ...
                    if (numeroHoras.doubleValue() >= 1) {
                        ...
                        //aplica el descuento
                        valorManoObra = valorManoObra.add(((valorManoObra.multiply(item.getDescuentoMano())).divide(BigDecimal.valueOf(Long.valueOf("100")))).negate());
                        //
                        item.setValorTotalManoObra(valorManoObra);
                    } else {
                        numeroHoras = BigDecimal.ONE;
                        item.setCantidadHoraMillar(BigDecimal.ONE);
                        valorManoObra = valorManoObra.add(numeroHoras.multiply(BigDecimal.valueOf(item.getManoObra().getManCostoTiempo())));
                        //System.out.println("Mano de obra hora menor 1:: " + item.getManoObra().getManCostoTiempo());
                        //aplica el descuento
...
//                BigDecimal valorParcial = valorManoObra.add(valorPliegos);
            ...
            System.out.println("no calcula los valores");
        }
//        } catch (Exception e) {
//            System.out.println("ERROR " + e.getMessage());
//        }
...
//            if (Utilitario.validar.validadorDeCedula(buscarCliente))
    ...
    /*
     ...
     BigDecimal cantPapel = BigDecimal.ZERO;
     ...
     //                //System.out.println("unidades por forma " + calculo);
     valor.setUxf(calculo.multiply(BigDecimal.valueOf(1)));
     } else {
     ...
     //cantidad de papel
     //            BigDecimal cantidadPorHojas = valor.getCantidad().multiply(valor.getMotivo());
     //            BigDecimal cantidadPorHojasSobreUxf = cantidadPorHojas.divide(BigDecimal.valueOf(valor.getCortezPosibles().getCodFxp()), RoundingMode.UP);
     ...
     //            BigDecimal cantidadPapelFinal = cantidadPorHojasSobreUxf.add(maculaturaSobreFXP);
     //             valor.setPliegos(cantidadPapelFinal);
     //fin calculo de papel

     //numero de ejempleres en cada pliego de papel
     ...
     //            BigDecimal cantidadSobreUxf = valor.getCantidad().divide(cabidasPorCorte, 0, RoundingMode.UP);
     //sumo el despedicio
     //            BigDecimal cantidadSobreUxfMasCulatura = cantidadSobreUxf.add(valor.getMaculatura());
     //           
     //            BigDecimal cantidadSobreUxfMasCulaturaSoberFxp = cantidadSobreUxfMasCulatura.divide(BigDecimal.valueOf(valor.getCortezPosibles().getCodFxp()), 4, RoundingMode.FLOOR);
     //            BigDecimal cantidadSobreUxfMasCulaturaSoberFxpPorPliegos = cantidadSobreUxfMasCulaturaSoberFxp.multiply(valor.getMotivo());
     //            cantPapel = valor.getCantidad().add(valor.getMaculatura());
     //            BigDecimal auxPapel = cantPapel.multiply(valor.getMotivo().divide(valor.getUxf(), 4, RoundingMode.FLOOR));
     //            cantidadSobreUxfMasCulaturaSoberFxpPorPliegos.setScale(0, RoundingMode.UP);
     BigDecimal cantidadPorMotivo = valor.getCantidad().multiply(valor.getMotivo());
     cantidadPorMotivo.setScale(0, RoundingMode.UP);
     BigDecimal cantidadPorMotivoSobrecabidasPorCorte = cantidadPorMotivo.divide(cabidasPorCorte, 0, RoundingMode.UP);

     valor.setPliegos(cantidadPorMotivoSobrecabidasPorCorte.add(valor.getMaculatura()));
     //            cantPliegosImpresion = valor.getMotivo().divide(valor.getUxf(), 0, RoundingMode.UP);
     BigDecimal auxiliar = BigDecimal.ZERO;
     // BigDecimal auxiliar1 = BigDecimal.ZERO;
     //            BigDecimal sumaAnvReverso = BigDecimal.ZERO;
     auxiliar = cantidadPorMotivoSobrecabidasPorCorte.add(valor.getMaculatura());
     ...
     cantPliegosImpresion = auxiliar;
     valor.setPliegosImpresionCompaginado(cantPliegosImpresion);
     ...
     //                valor.setPlacas(BigDecimal.valueOf(valor.getColorAnverso()).add(BigDecimal.valueOf(valor.getColorReverso())));
...
//             valor.setPliegos(cantidadPapelFinal);
            //fin calculo de papel
            valor.setListaCabidas(servicioCabida.FindALlCabiPorUXF(Integer.parseInt(valor.getUxf().toString())));
            BigDecimal cantidadSobreUxf = valor.getCantidad().divide(valor.getUxf(), 0, RoundingMode.UP);
            BigDecimal cantidadSobreUxfMasCulatura = cantidadSobreUxf.add(valor.getMaculatura());
            BigDecimal cantidadSobreUxfMasCulaturaSoberFxp = cantidadSobreUxfMasCulatura.divide(BigDecimal.valueOf(valor.getCortezPosibles().getCodFxp()),0,RoundingMode.FLOOR);
            BigDecimal cantidadSobreUxfMasCulaturaSoberFxpPorPliegos = cantidadSobreUxfMasCulaturaSoberFxp.multiply(valor.getMotivo());
            cantPapel = valor.getCantidad().add(valor.getMaculatura());
            BigDecimal auxPapel = cantPapel.multiply(valor.getMotivo().divide(valor.getUxf(), 4, RoundingMode.FLOOR));
            valor.setPliegos(cantidadSobreUxfMasCulaturaSoberFxpPorPliegos);
            cantPliegosImpresion = valor.getMotivo().divide(valor.getUxf(), 0, RoundingMode.UP);
            BigDecimal auxiliar = BigDecimal.ZERO;
            // BigDecimal auxiliar1 = BigDecimal.ZERO;
            BigDecimal sumaAnvReverso = BigDecimal.ZERO;
...
//        window.doModal();
    ...
    public void buscarClienteEnLista() {
        ...
        org.zkoss.zul.Window window = (org.zkoss.zul.Window) Executions.createComponents(
                "/cotizacion/buscarClienteCompaginado.zul", null, null);
        window.doModal();
